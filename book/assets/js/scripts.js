(async function () {
      ...
      if (mutation.type === "childList") {
        ...
        if (mutation.target.nodeName === "TBODY") {
          if (mutation.target.childNodes.length === 1) {
            // mutation.target.appendChild(bookApp.makeRow("empty-data"));
          }
        }
  ...
  if (viewTableBody.childElementCount === 0) {
    // viewTableBody.appendChild(bookApp.makeRow("empty-data"));
  }

  document.getElementById("view-table-body").addEventListener("click", function (e) {
...
document
  .querySelector("#add-book-shortcut")
